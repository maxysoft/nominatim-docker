...
type Config struct {
	...
	GunicornBind    string
	GunicornWorkers int
	// GunicornGracefulTimeout is Gunicorn's drain deadline in seconds; the
	// SIGKILL deadline of every child follows it (ShutdownGrace).
	GunicornGracefulTimeout int
...
func Load() (*Config, error) {
	...
	if c.GunicornGracefulTimeout, err = envInt("GUNICORN_GRACEFUL_TIMEOUT", 30); err != nil {
		return nil, err
	}
...
func (c *Config) Validate() error {
	...
	if c.GunicornGracefulTimeout < 0 {
		return fmt.Errorf("GUNICORN_GRACEFUL_TIMEOUT must be >= 0, got %d", c.GunicornGracefulTimeout)
	}
	switch c.UpdateMode {
...
// ShutdownGrace is how long a child may drain after SIGTERM before it is
// killed: past Gunicorn's own deadline, so it is never cut short.
func (c *Config) ShutdownGrace() time.Duration {
	return time.Duration(c.GunicornGracefulTimeout)*time.Second + 5*time.Second
}
...
func (c *Config) LibpqURL(user, password, database string) string {
	...
	// url.URL escapes '@', ':' and '/' in the userinfo: pgx, like libpq, ends
	// it at the first '@'. JoinHostPort brackets an IPv6 literal.
	u := url.URL{
		Scheme:   "postgres",
		User:     url.UserPassword(user, saslprep(password)),
		Host:     net.JoinHostPort(c.PostgresHost, strconv.Itoa(c.PostgresPort)),
		Path:     "/" + database,
		RawQuery: "sslmode=" + c.PostgresSSLMode + "&application_name=nominatim-ctl",
	}
	return u.String()
}
...
func WriteEnvFile(c *Config, uid, gid int) error {
	...
	stale, _ := filepath.Glob(filepath.Join(filepath.Dir(path), ".env.tmp-*"))
	for _, p := range stale {
	...
	if errors.Is(err, syscall.EBUSY) {
		...
		err = writeInPlace(path, content, uid, gid)
	}
	if err != nil {
		return fmt.Errorf("writing %s: %w", path, err)
	}
